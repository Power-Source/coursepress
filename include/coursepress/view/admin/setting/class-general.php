...
class CoursePress_View_Admin_Setting_General {
	...
	public static function add_tabs( $tabs ) {
		...
		$tabs['general'] = array(
			'title' => __( 'Allgemeine Einstellungen', 'cp' ),
			'description' => __( 'Konfiguriere die allgemeinen Einstellungen für CoursePress.', 'cp' ),
			'order' => 0,// first tab
		);
	...
	public static function return_content( $content, $slug, $tab ) {
		...
		$pages_args = array(
			...
			'echo' => 0,
			'show_option_none' => __( 'Virtuelle Seite verwenden', 'cp' ),
			'option_none_value' => 0,
		...
		' . wp_nonce_field( 'update-coursepress-options', '_wpnonce', true, false ) . '
				...
				<p class="description">' . sprintf( __( 'Ein Slug ist ein paar Wörter, die einen Beitrag oder eine Seite beschreiben. Slugs sind normalerweise eine URL-freundliche Version des Beitragstitels (der automatisch von WordPress generiert wurde), aber ein Slug kann alles sein, was Du möchtest. Slugs sollen mit %s verwendet werden, da sie helfen, den Inhalt unter der URL zu beschreiben. Der Beitragsslug ersetzt den %s-Platzhalter in einer benutzerdefinierten Permalink-Struktur.', 'cp' ), '<a href="options-permalink.php">Permalinks</a>', '<strong>"%posttitle%"</strong>' ) . '</p>
				<div class="inside">
					...
					<table class="form-table slug-settings">
						<tbody>
							...
							<tr valign="top" class="break">
								<th scope="row">' . esc_html__( 'Kurs Kategorie Slug', 'cp' ) . '</th>
								<td>' . esc_html( trailingslashit( home_url() ) . trailingslashit( esc_html( CoursePress_Core::get_setting( 'slugs/course', 'courses' ) ) ) ) . '
									...
									<p class="description">' . esc_html__( 'Deine Kurskategorie-URL wird wie folgt aussehen: ', 'cp' ) . trailingslashit( esc_url( home_url() ) ) . esc_html( CoursePress_Core::get_setting( 'slugs/course', 'courses' ) . '/' . CoursePress_Core::get_setting( 'slugs/category', 'course_category' ) ) . esc_html__( '/deine-kategorie/', 'cp' ) . '</p>
								</td>
							</tr>
							<tr valign="top" class="break">
								<th scope="row">' . esc_html__( 'Einheit Slug', 'cp' ) . '</th>
							...
							<tr valign="top" class="break">
								<th scope="row">' . esc_html__( 'Kurs Benachrichtigungen Slug', 'cp' ) . '</th>
								<td>' . trailingslashit( esc_url( home_url() ) ) . trailingslashit( esc_html( $my_course_prefix ) ) . '
							...
							<tr valign="top" class="break">
								<th scope="row">' . esc_html__( 'Kurs Diskussionen Slug', 'cp' ) . '</th>
								<td>' . trailingslashit( esc_url( home_url() ) ) . trailingslashit( esc_html( $my_course_prefix ) ) . '
							...
							<tr valign="top">
								<th scope="row">' . esc_html__( 'Neue Kurs Diskussion Slug', 'cp' ) . '</th>
							...
							<tr valign="top" class="break">
								<th scope="row">' . esc_html__( 'Kurs Noten Slug', 'cp' ) . '</th>
								<td>' . trailingslashit( esc_url( home_url() ) ) . trailingslashit( esc_html( $my_course_prefix ) ) . '
							...
							<tr valign="top" class="break">
								<th scope="row">' . esc_html__( 'Kurs Arbeitsbuch Slug', 'cp' ) . '</th>
								<td>' . trailingslashit( esc_url( home_url() ) ) . trailingslashit( esc_html( $my_course_prefix ) ) . '
							...
							<tr valign="top">
								<th scope="row">' . esc_html__( 'Login Seite', 'cp' ) . '</th>
							...
							<tr valign="top">
								<th scope="row">' . esc_html__( 'Signup Seite', 'cp' ) . '</th>
							...
							<tr valign="top">
								<th scope="row">' . esc_html__( 'Student Dashboard Seite', 'cp' ) . '</th>
								<td>' .  self::add_page_dropdown_description( $page_dropdowns['student_dashboard'], 'student_dashboard' ). '</td>
							</tr>

							<tr valign="top" class="break">
								<th scope="row">' . esc_html__( 'Student Einstellungen Slug', 'cp' ) . '</th>
								<td>' . trailingslashit( esc_url( home_url() ) ) . '
							...
							<tr valign="top">
								<th scope="row">' . esc_html__( 'Student Einstellungen Seite', 'cp' ) . '</th>
		...
		$content .= '
				...
				<!-- THEME MENU ITEMS -->
				<h3 class="hndle" style="cursor:auto;"><span>' . esc_html__( 'Theme Menüelemente', 'cp' ) . '</span></h3>
				<div class="inside">
					<table class="form-table">
						<tbody>
							<tr valign="top">
								<th scope="row">
								' . esc_html__( 'Menüpunkte anzeigen', 'cp' ) . '
									<a class="help-icon" href="#"></a>
									<div class="tooltip hidden">
										<div class="tooltip-before"></div>
										<div class="tooltip-button">&times;</div>
										<div class="tooltip-content">
											' . __( '<div>Füge die Standard-CoursePress-Menüpunkte (Kurse, Student Dashboard, Abmelden) zum <strong>Hauptmenü</strong> hinzu.</div><div>Elemente können auch über Design > Menüs und das CoursePress-Panel hinzugefügt werden.</div>', 'cp' ) . '
										</div>
									</div>
								</th>
								<td>';
		...
		$content .= '<input type="checkbox" name="coursepress_settings[general][show_coursepress_menu]" ' . $checked . ' />';
				...
				<div class="inside">
					<table class="form-table">
						<tbody>
							<tr valign="top">
								<th scope="row">
								' . esc_html__( 'Benutzerdefiniertes Anmeldeformular verwenden', 'cp' ) . '
									<a class="help-icon" href="#"></a>
									<div class="tooltip hidden">
										<div class="tooltip-before"></div>
										<div class="tooltip-button">&times;</div>
										<div class="tooltip-content">
											' . __( 'Verwendet ein benutzerdefiniertes Anmeldeformular, um die Schüler auf der Front-End-Seite Ihrer Website zu halten.', 'cp' ) . '
										</div>
									</div>
								</th>
								<td>';
