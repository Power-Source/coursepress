...
class CoursePress_View_Admin_Setting_Slugs extends CoursePress_View_Admin_Setting_Setting {
	...
	public static function add_tabs( $tabs ) {
		...
		$tabs[ self::$slug ] = array(
			'title' => __( 'Slugs', 'cp' ),
			'description' => sprintf( __( 'Ein Slug ist ein paar Wörter, die einen Beitrag oder eine Seite beschreiben. Slugs sind normalerweise eine URL-freundliche Version des Beitragstitels (der automatisch generiert wurde), aber ein Slug kann alles sein, was Du möchtest. Slugs sollen mit %s verwendet werden, da sie helfen, den Inhalt unter der URL zu beschreiben. Der Beitragsslug ersetzt den %s-Platzhalter in einer benutzerdefinierten Permalink-Struktur.', 'cp' ), '<a href="options-permalink.php">Permalinks</a>', '<strong>"%posttitle%"</strong>' ),
			'order' => 3,
		);
	...
	public static function return_content( $content, $slug, $tab ) {
		...
		$content .= self::row(
			...
			esc_html__( 'Deine Kurs-URL wird wie folgt aussehen: ', 'cp' ) . esc_html( $home_url ) . esc_html( CoursePress_Core::get_setting( 'slugs/course', 'courses' ) ) . esc_html__( '/my-course/', 'cp' )
		);

		$content .= self::row(
			__( 'Kurs Kategorie Slug', 'cp' ),
			...
			esc_html__( 'Deine Kurskategorie-URL wird wie folgt aussehen: ', 'cp' ) . $home_url . esc_html( CoursePress_Core::get_setting( 'slugs/course', 'courses' ) . '/' . CoursePress_Core::get_setting( 'slugs/category', 'course_category' ) ) . esc_html__( '/deine-kategorie/', 'cp' )
		);
		...
		$content .= self::row(
			__( 'Kurs Benachrichtigungen Slug', 'cp' ),
		...
		$content .= self::row(
			__( 'Kurs Neue Diskussion Slug', 'cp' ),
		...
		$content .= self::row(
			__( 'Einschreibungsprozess Slug', 'cp' ),
